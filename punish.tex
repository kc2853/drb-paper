...
\end{itemize}
In a typical setting, the idea behind DKG is that it allows $t + 1$ (but not less) out of $n$ nodes to jointly \textit{use} $sk$ without necessarily \textit{knowing} $sk$ (though this is not the case in Escrow-DKG). See Appendix~\ref{appendix:dkg} for details.
