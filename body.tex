...
\item \textbf{Aggregate.} The final randomness is $\drboutput_r = \prod_{j \in \mathcal{C}_r} h^{s^{(j)}}$.
\end{enumerate}
...
\noindent\textbf{Random Selection (RS).} A second example is \textit{random selection} (RS), which uses some public randomness (most commonly the last beacon output $\drboutput_{r - 1}$) to derive $\mathcal{C}_r$.
...
Randomized selection means that some nodes may, in theory, never be selected and therefore may never be able to provide entropy contribution. A more serious concern is that an adversary can attempt to bias, via grinding attack, $\drboutput_r$ in order to bias $\mathcal{C}_{r + 1}$ (which can bias $\drboutput_{r + 1}$). In the worst case, this can lead to a vicious cycle in which an adversary controlling enough nodes on the current committee to manipulate the beacon output can ensure it will also control enough nodes on the next committee, and so on ad infinitum.
...
Given a committee $\mathcal{C}_r$, the issue then shifts to outputting $\drboutput_r$. While a typical commit-reveal-recover run among nodes in $\mathcal{C}_r$ may be sufficient to realize a DRB, other approaches provide different tradeoffs. Largely, we classify these variations into two: one that requires \textit{fresh} (independently generated on the spot) per-node entropy (contribution) and one that combines previous beacon output with \textit{precommitted} (independently generated but precommitted, hence ungrindable) per-node entropy.
...
\noindent\textbf{Share-Reconstruct-Aggregate.} In Ouroboros, nodes in $\mathcal{C}_r$ (i.e. slot leaders of epoch $r$) perform a RandShare-style share-reconstruct-aggregate using PVSS to output $\drboutput_r$. RandHound uses a similar approach, facilitated by a round leader.\\
...
\todo{elaborate}
\begin{enumerate}
\item Full information model
    \begin{itemize}
    \item Seminal work by Ben-Or and Linial~\cite{ben1985collective,ben1989collective}
    \item Computationally unbounded adversary + single broadcast channel
    \item Honest majority assumption~\cite{saks1989robust,boppana2000perfect}
    \item Types of adversaries: static, adaptive, variants of adaptive
    \item BUT definition of security is a lot more tolerant with respect to bias, asymptotic (as opposed to exact) bounds, output is only a bit in the first place
    \end{itemize}
\item Dishonest majority (using cryptography + computationally bounded)
    \begin{itemize}
    \item Cleve lower bound~\cite{cleve1986limits}
    \item Timed-based cryptography + delay functions circumvent the lower bound due to:
    \item 1. Timed commitments---no need for "default bit" mechanism if the other node fails or withholds
    \item 2. VDFs---a Byzantine adversary is unable to grind through possibilities based on outputs from honest nodes if a VDF is applied
    \end{itemize}
\end{enumerate}

\subsection{Grinding Attack}
In a grinding attack, an adversary can search over many possible inputs to broadcast in a given round, with the possibility of influencing the outcome. While grinding attacks are not a threat in commit-reveal-recover variants or protocols with no marginal entropy, they are a valid concern in committee-based protocols involving random selection (RS), a private lottery, or a beacon output generated by combining precommitted per-node entropy with $\drboutput_{r - 1}$. The idea is that an adversary can grind to produce a biased $\mathcal{C}_r$, $\drboutput_r$, or both (leading to a vicious cycle in the worst case).
