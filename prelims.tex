...
We formalize the security properties of an ideal DRB as follows:\footnote{We present a game-based security definition here. Other formulations, such as ideal functionalities as used in UC-security\todo{cite}, are possible. We present game-based security here as it is simple and most commonly used~\todo{Check this? does anybody do UC-security?}.}
...
\item Liveness\todo{is this part of unbias game?}. We define liveness~\cite{guo2020secRand} by requiring that the advantage of $\adv$ denoted by $Pr[\drboutput_\epochv = \text{$\perp$}]$ (i.e. the probability that the honest beacon output at the end of \epoch $\epochv$ is null) is negligible, given a DRB that runs among honest participants and $\adv$.

\item Unpredictability.
We define three types of unpredictability.
...
\begin{itemize}
\item A DRB is \textit{\intraunpredictable} if $\adv$ participating in $\drb^\epochv$ (Figure~\ref{figure:unbias}) can predict $\drboutput_\epochv$ before $\drbtimeone$ with negligible probability.
...
\item A DRB is \textit{$d$-\interunpredictable} if $\adv$ can predict $\drboutput_{\epochv + d}$ before $\drbtimetwo$ with negligible probability.
\end{itemize}
\end{enumerate}
\end{definition}
