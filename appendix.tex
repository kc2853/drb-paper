...
%             \item \textit{Case 2.} Otherwise, we have a $(n-t) \times \ell$ matrix $T$ with rows indexed by the participants $P_a \in \mathcal{C}$ where the row corresponding to $P_a$ is $(h^{s^{(a)}_{0}}, \ldots, h^{s^{(a)}_{\ell-1}})$. The final $\ell \times \ell$ randomness matrix $\drboutput$ is computed as $\drboutput = M \diamond T$ by applying FFTE to each column $T^{(j)}$ of $T$ producing column $\drboutput^{(j)}$ of $\drboutput$.
%         \end{itemize}
% \end{enumerate}
