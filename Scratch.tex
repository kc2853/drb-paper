...
Since nothing can be gained from failing Commit, the malicious behavior of participants is limited to withholding shares to abort the protocol during Reveal or colluding to learn the secret before Reveal.
...
\textbf{Albatross}\\
Albatross extends Scrape to provide an improved amortized computational complexity of $\theta(\log n)$ exponentiations per random value per participant in the worst case (as compared to $\theta(n^2)$ exponentiations required in Scrape) by generating a batch of $\theta(n^2)$ uniformly random values at the end of each round by using packed Shamir secret sharing based PVSS to share $\ell \le n-t$ secrets instead of 1 without increasing the computational cost (resulting in a total of $n \times \ell$ secrets being shared) and linear $t$-resilient functions to extract final randomness of $\ell^2$ elements. However, it can only tolerate up to $t = n/2 - \theta(n)$ corrupt participants (as compared to $t = n/2$ in Scrape). See Appendix \ref{appendix:albatross} for more details about the protocol.\\
