...
\begin{itemize}
    \item $\mathsf{DKG}(1^\lambda, t, n) \rightarrow (\vec{SK}, \vec{PK}, pk)$ runs DKG protocol and generates the common public key $pk$, individual secret keys $\vec{SK}$ and the corresponding individual public keys $\vec{PK}$.
     ...
     \item $\mathsf{ShareDec}(pk, sk_i, c) \rightarrow \mu_i$ takes ciphertext $c = (A, B)$, the individual secret key $sk_i$ and the group public key $pk$ as input and generates the decrypted share $\mu_i = sk_i \cdot A$.
