...
RandShare uses VSS as a subprotocol, extended by adopting the concept of barrier, a specific point in the protocol execution only after which the protocol is guaranteed to complete successfully. In RandShare, the barrier is reached when the first honest node reveals the shares he holds. Because of the secret sharing threshold of $t + 1$, the dishonest participants cannot recover the honest participants’ secrets before the barrier. The protocol output cannot be changed after the barrier, and all honest participants eventually output the previously fixed value, regardless of the adversary’s behavior.\\
