...
\begin{enumerate}
    ...
    The final beacon output is calculated as $\mathcal{O}_r = H(y_r) $, which is then posted and can be efficiently verified using $\pi$ through $VDF.Verify$.
\end{enumerate}
...
As proposed in \cite{bunz2017proofs}, public sources of entropy such as stock prices or PoW blockchains which are otherwise susceptible to manipulation from insiders (high frequency traders and miners respectively) can be used to extract $seed_r$ which is fed into $VDF.Eval$ (similar to Unicorn++) as follows:
$$seed_r = \mathsf{Ext}_k(e_r)$$
where $\mathsf{Ext}_k$ is an extractor \cite{bonneau2015bitcoin} that takes as inputs an $n$-bit $e_r$ from some biasable external beacon and an $\ell$-bit key $k$ and returns an $m$-bit output $seed_r$ of high entropy where $m < n$.
...
\end{itemize}
Escrow-DKG is usually used to generate keys for secret sharing. EVR adapts it to generate random numbers with the algorithms $EDKG = (EscrowEnroll, Commit, Reveal, EscrowVerify)$. $EDKG.EscrowEnroll$ is used by participants to register with the escrow by submitting a deposit along with the commitment to a secret. $EDKG.Commit$ proceeds in a similar way as $VSS.ShareGen$. $EDKG.Reveal$ reconstructs the shared secret using valid shares via Lagrange interpolation. $EDKG.EscrowVerify$ is used by the escrow to verify the consistency of the recovered secret with the published shares. See Appendix \ref{appendix:edkg} for details.
...
\begin{enumerate}
    ...
    \item \textbf{Reconstruction.} Every participant verifies the correctness of decryption using $PVSS.ShareVerify$. Once $t+1$ valid group secret shares are distributed, the complete group secret $gs$ is reconstructed through $PVSS.Recon$. The final random output for the round, $\mathcal{O}_r = H(gs)$.
\end{enumerate}
...
The following tuple of DVRF algorithms differentiates DDH-DVRF.
\begin{enumerate}
...
\cmidrule{1-5}
\multirow{6}{*}{\spheading[0.465\textwidth]{\mbox{Step 1: Subset Selection}}} & \multirow{3}{*}[-2.8cm]{Public} & RR & \begin{tabular}{@{}l@{}}\textbf{BRandPiper}\\\textit{Step 1}: Node $i \equiv r \pmod n$\\\textit{Step 2}: Share-aggregate-reconstruct\end{tabular} & \\
\cmidrule{3-5}
