...
\begin{enumerate}
    ...
    \item \textbf{Inform.} Any colluding participant that preemptively knows $\mathcal{O}_r$ can inform $\mathcal{G}$ to earn a high informing reward ($\ell$) via collective punishment. Due to this phase, the incentive is that no one should collude.
...
While a node's malicious behaviors in EVR are limited to withholding to abort the protocol during Reveal or colluding to learn $\mathcal{O}_r$ before Reveal, its security comes from the fact that both are disincentivized. First (to disincentivize biasability), setting $P$ to $n - t$ makes withholding unprofitable to $n - t$ or more nodes that can collectively withhold to abort the protocol, as profit they would make would not be greater than what they would lose by $\mathcal{G}$ slashing all the deposits. Second (to disincentivize predictability), setting the informing reward $\ell$ to $n$ makes informing more profitable than any illicit profit and thus disincentivizes any nodes colluding to learn $\mathcal{O}_r$ before others. The idea is that any coalition of malicious actors (deviating from EVR) is going to have yet another malicious actor (deviating from the coalition to earn $\ell$).
...
We provide Table \ref{table:comparison} to offer an overall comparison of DRBs. \textit{Fault Tolerance} indicates the minimum number of faulty nodes that can abort a protocol (after the initial setup). Protocols with \textit{Independent Participation} allows a node to contribute to beacon output without the knowledge of other nodes in advance. However, it differs from a permissionless setting in the sense that a node may still have to register in advance to allow verification of its contribution.

\textit{Verification Complexity} refers to the computational cost for a passive node to verify a beacon output. We exclude the cost associated with the initial setup for both verification and communication complexities. Verification complexities for threshold-based protocols are computed assuming a cost of $O(n)$ per Lagrange interpolation or PVSS run (based on Scrape \cite{cascudo2017scrape}). \textit{Communication Complexity} concerns bitwise point-to-point communication between nodes by default. We also consider a public bulletin board (PBB) as a reliable and stable information exchange medium in protocols where it is intrinsic to the protocol's construction. For PBB, we assume both the bitwise writing cost (amount of data posted to PBB) and the reading cost (by all nodes where each node only reads data relevant to it) contribute to the total cost. In the absence of PBB, Byzantine consensus incurs a cost of $O(n^2)$ per decision by default.

\textit{Max Damage} refers to the maximum damage possible when $n - 1$ adversarial nodes cooperate to predict or bias the beacon output. For escrow-based protocols, we assume the adversaries are rational. \textit{Recovery} refers to the communication cost associated with recovering from an adversarial corruption. Regenerating keys (e.g. $\mathsf{PVSS.KeyGen}$ or for private lottery schemes) and $\mathsf{VDF.Setup}$ incur $O(n^3)$ recovery cost without PBB (and $O(n^2)$ with PBB) while we assume each DKG incurs $O(n^4)$ recovery cost.
% \iffalse
...
A $(t, n)$-threshold ElGamal cryptosystem \cite{desmedt1990Threshold, fouque2001threshold, cherniaeva2019homomorphic}, with an elliptic curve $E$ over $\mathbb{F}_p$ and its cyclic subgroup $\mathbb{G}_q$ of prime order $q$ with generator $G$, is implemented as follows.
\begin{itemize}
    \item $\mathsf{DKG}(1^\lambda, t, n) \rightarrow (sk_i, pk_i, pk)$ runs a typical DKG.
