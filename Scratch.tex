...
\begin{itemize}
    ...
    \item $\mathsf{ShareDec}(pk, sk_i, pk_i, c) \rightarrow d_i$ generates decryption share $d_i$ for ciphertext $c$ using individual key pair $(sk_i, pk_i)$ and $pk$.
...
\label{appendix:thresholdEnc}
    ...
    \begin{itemize}
        ...
        \item Robustness. For any ciphertext $c$ and any two $(t + 1)-$subsets of decryption shares $d \ne d'$ such that $\mathsf{Rec}(pk, c, d) \ne {Reject}$ and $\mathsf{Rec}(pk, c, d') \ne {Reject}$, then it holds that $\mathsf{Rec}(pk, c, d) = \mathsf{Rec}(pk, c, d')$.
