...
\textbf{Public External Beacons.} The other alternative is to construct a beacon using publicly available external sources of entropy such as stock market data \cite{clark2010use} or Proof of Work (PoW) blockchains like Bitcoin \cite{nakamoto2019bitcoin, bentov2016bitcoin, han2020randchain} as formalized in \cite{bonneau2015bitcoin}. However, such beacons are vulnerable to malicious insiders.
...
As long as $T$ is longer than the duration of Collect phase (including $\Delta$), even the last entropy provider cannot bias the final beacon output. Also notably, Unicorn++ is unbiasable by an adversary that controls $n - 1$ of the participants, as the entropy from even one honest participant would require computation of $\mathsf{VDF.Eval}$ from scratch.

Additionally, as proposed in \cite{bunz2017proofs, bonneau2015bitcoin}, beacons using stock prices \cite{clark2010use} or PoW blockchains \cite{bonneau2015bitcoin} which are otherwise susceptible to manipulation can be used to supply $seed_r$ in Collect phase.
...
RandRunner generates fresh randomness rapidly with only $O(n)$ communication complexity in the optimistic case. Adversarial leaders and network delay ($\Delta$) increases the round duration to $T$ and the communication complexity to $O(n^2)$. Because $\mathcal{O}_r$ can be computed independently (with delay $T$), RandRunner retains liveness even with a dishonest majority and when network connectivity breaks down completely. The deterministic nature of RandRunner also prevents an adversary from biasing the beacon outputs. However, RandRunner produces pseudorandom numbers (and not uniform random numbers) and trails in terms of the quality of randomness. Assuming a $t$-limited adversary with relative computational advantage of $\alpha$ with respect to honest nodes, RandRunner provides $d$-unpredictability where $d = t \cdot \alpha$ rounds if the honest nodes collectively progress faster than the malicious nodes and the following inequality holds: $$n-t > t \cdot \alpha$$ This is reduced to standard majority assumption $n > 2t$ only when $\alpha = 1$.
