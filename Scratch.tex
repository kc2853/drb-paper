...
\label{section:vdf}
One way to prevent the last revealer attack is to add a delay function after collecting entropy, making it slow to compute the final beacon output. As long as the delay is suitably long, none of the participants would be able to determine the final beacon output and manipulate or withhold their contributions to bias it. A verifiable delay function (VDF) \cite{boneh2018verifiable} can be used to accomplish this.
...
\begin{itemize}
    ...
    \item $\mathsf{Enc}(pk, m) \rightarrow c$ encrypts the message $m$ with group public key $pk$ and outputs the ciphertext $c$.
    \item $\mathsf{ShareDec}(pk, sk_i, pk_i, c) \rightarrow d_i$ generates the decryption share $d_i$ for ciphertext $c$ using the individual keys $(sk_i, pk_i)$ and $pk$.
    \item $\mathsf{Rec}(pk, \vec{PK}, c, \vec{d} )\rightarrow \{m, Reject\}$ is a recovery algorithm that takes $pk$, a ciphertext $c$, and a subset of $t + 1$ valid decryption shares $\vec{d} = \{d_1, d_2,\ldots, d_{t + 1}\}$ together with the public keys $\vec{PK} = \{pk_1, pk_2,\ldots, pk_{t + 1}\}$ and outputs the message $m$ via one Lagrange interpolation or $Reject$.
\end{itemize}
\end{definition}
...
\begin{enumerate}
    ...
    \item \textbf{Disclosure.} Each key holder $P_i$ uses $\mathsf{ThrEnc.ShareDec}$ to generate a decryption share $d_i$, published along with a proof of correct decryption $\pi_{DLEQ}^{(i)}$ (Appendix \ref{appendix:dleq}). When $t + 1$ decryption shares are published and verified, nodes use $\mathsf{ThrEnc.Rec}$ to output $\mathcal{O}_r = m$.
\end{enumerate}
...
In this paper, we provide an overview and comparison of various DRB protocols which provide a range of cost and quality trade-offs under different setups, assumptions and adversarial models. We further describe a modular framework comprising of two components: selection of entropy providers and beacon output generation, which can be used to analyze any practical DRB's design. Based on our systematization, we provide the following insights:
\begin{itemize}
    \item PVSS/VSS based protocols are suitable when high quality of unbiased randomness is to be generated with a dynamic but small set of entropy providers. To scale it to a larger set of nodes, various techniques to select a subset of entropy providers can be used.
    \item Protocols requiring DKG for initial setup (DVRF, threshold encryption based) works well with a larger, but relatively static set of entropy providers due to the high cost associated with it.
    ...
    \item VRF based protocols are suitable for generating quick biasable pseudorandomness with a large dynamic set of entropy providers and is commonly used in protocols that require randomness as part of their overall construction.
...
\end{itemize}
While significant developments have been made to address the problem of generating unbiasable randomness, we identify the following areas which could benefit from further research. 
\begin{itemize}
    \item Extending/modifying existing protocols to deal with asynchrony.
    \item Combining multiple stand-alone beacons in a modular way to build a heterogeneous beacon. Mt.Random \cite{cascudomt}, a multi-tiered randomness beacon providing different trade-off between cost and quality of randomness explores this idea.
...
\begin{itemize}
    \item $\mathsf{DKG}(1^\lambda, t, n) \rightarrow (\vec{SK}, \vec{PK}, pk)$ runs DKG protocol and generates the common public key $pk$, individual secret keys $\vec{SK}$ and the corresponding individual public keys $\vec{PK}$.
     ...
     \item $\mathsf{ShareDec}(pk, sk_i, c) \rightarrow D_i$ takes ciphertext $c = (A, B)$, the individual secret key $sk_i$ and the group public key $pk$ as input and generates the decrypted share $D_i = sk_i \cdot A$.
