...
Extending commit-reveal by adding a delay, Unicorn \cite{lenstra2015random} uses the Sloth function (a VDF precursor involving modular square roots in modulo prime), which can be replaced with a VDF to desirably achieve an exponential gap between computation and verification times. We refer to this VDF-based Unicorn as Unicorn++. In Unicorn++ (as well as Unicorn), participants can skip the commit step and send their contributions directly. It runs as follows.
\begin{enumerate}
