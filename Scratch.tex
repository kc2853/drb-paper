...
The main building block of Scrape is its PVSS scheme (see Appendix \ref{appendix:scrapePVSS} for details). The initial setup for Scrape requires generating public keys $pk_i$ for each of the $n$ participants using $PVSS.KeyGen$. 
...
\item $PartialVerify(pk_i, x, y_i, \pi_i)$ is equivalent to $DLEQ\text{-}Verify(g, pk_i, H(x), y_i, \pi_i)$ and verifies the correctness of the $PartialEval$ algorithm using $\pi_i$.
...
\item $PartialVerify(pk_i, x, y_i, \pi_i)$ is equivalent to $DLEQ\text{-}Verify(g_1, pk_i, H_1(x), y_i, \pi_i)$ and verifies the correctness of the $PartialEval$ algorithm using $\pi_i$.
