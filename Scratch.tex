...
\begin{itemize}
\item $\mathcal{P} = \{P_1, P_2, ..., P_n\}$ but wlog $\{1, 2, ..., n\}$
\item $n$ participants, up to $t$ faulty nodes engaging in incorrect behaviors during a protocol run, $t$-limited
\item honest, Byzantine, corrupted
...
\item $\mathcal{A}$: computationally bounded, cannot break hash function/digital sig/discrete log problem/etc.
\item 3 things: omitting a message in the protocol, sending invalid, colluding to coordinate an attack
\item $g, h \in \mathbb{G}$
\item $H(x)$ random oracle
\item synchronous network by default and definition
\item $\mathcal{O}_r$: beacon output in round $r$
\end{itemize}
...
The work of \cite{lenstra2015random} proposes a solution to the last revealer attack in commit-reveal schemes using the Sloth function (a VDF precursor involving modular square roots in modulo prime) via Unicorn, a protocol that could in fact be extended to Unicorn++ by utilizing a VDF in place of Sloth. In doing so, Unicorn++ desirably achieves an exponential gap between computation and verification times unlike Unicorn. Besides the tool used (VDF vs Sloth), Unicorn++ and Unicorn share the same fundamental concept. Namely, each participant can skip the commit step and post their $r_i$ directly such that $seed = H(r_1,\ldots, r_n)$ is passed through $VDF.Eval$. \todo{this should be spelled out as explicit protocol steps, this is way too informal} The final beacon outcome is the hash of the output of $VDF.Eval$, which is then posted and can be efficiently verified. As long as the delay parameter $T$ is longer than the time period during which the values may be submitted, \todo{need to tie this to the underlying network assumptions} even the last participant to submit their $r_i$ cannot predict the final beacon output. Such a beacon is unbiasable (and unpredictable) by an adversary that controls $n-1$ of the participants and uses only two rounds of communication. \joenote{Why? howis this proved?} This stands in contrast to other threshold based secret sharing schemes (discussed later) which require multiple rounds of communication and an honest majority.
...
RandRunner \cite{schindler2021randrunner} provides a novel way to use VDFs to derive distributed randomness. It builds a deterministic chain of VDF outputs on an initial random seed. Assuming the initial seed is unbiased, the deterministic nature of VDF makes the beacon output unbiasable and the sequential work needed to compute the VDF makes it unpredictable (for time given by delay parameter $T$). RandRunner uses \emph{strongly unique trapdoor VDFs}.\\\\
\textbf{Trapdoor VDFs}\\
Trapdoor VDFs, initially described in \cite{wesolowski2019efficient}, extends traditional VDFs by providing an alternate way to efficiently evaluate the VDF. Trapdoor VDFs has the following additional properties:
\begin{itemize}
...
\end{itemize}
Trapdoor VDFs can be described by the algorithms $TVDF = (Setup,VerifySetup, TrapdoorEval, Eval, Verify)$. See Appendix \ref{appendix:tvdf} for details. Wesolowski’s VDF \cite{wesolowski2019efficient} does not achieve strong uniqueness because knowing the trapdoor allows an adversary to forge ``valid'' proofs for different invalid output values, thus breaking uniqueness and sequentiality properties. As a result, RandRunner uses Pietrzak's VDF \cite{pietrzak2018simple}.

RandRunner starts with a one-time Setup and Bootstrapping phase which proceeds as follows:\\

\textbf{Setup:} Each participant $i$ executes $TVDF.Setup(\lambda,T)$ to compute its public parameters $pp_i$ and the corresponding secret trapdoor $sk_i$. The public parameters are exchanged with each other while keeping the individual trapdoors secret. At the end, each participant should have the same set $\mathcal{P} = \{ pp_1, pp_2, \ldots pp_n \}$ containing the public parameters of all participants. Each participant then verifies the exchanged parameters by executing $TVDF.VerifySetup(\lambda, pp_i)$ for all $pp_i \in \mathcal{P}$. \joenote{what does this actually verify and why is it necessary?}

\textbf{Bootstrapping:} After all the public parameters are set up, the protocol is bootstrapped with an initial random beacon value $\mathcal{O}_0$ broadcast to all the participants. It is used to select the leader for the first protocol round and serves as the input to the first (leader’s) VDF being evaluated.

\textbf{Execution:} The execution of the protocol proceeds in rounds. Every round $r$ has a unique leader whose duty is to advance the protocol into the next round. A leader can be selected either in a round-robin style (i.e. taking turns in some permuted order based on $\mathcal{O}_0$) or in a way that involves randomized sampling (i.e. using the previous round’s randomness as seed). The implications of each will be discussed in Section \ref{subsubsection:public-subset-selection}.\joenote{what are the implications of each?} The execution can proceed in two ways depending on the honesty of the leader.

    \textbf{Honest Leader (Common Case):} The leader advances the protocol into next round as follows:
    \begin{itemize}
        \item \joenote{this might be too much detail, this step} Previous random beacon value $\mathcal{O}_{r-1}$ is mapped to the input space of its VDF using a hash function $H_{in} : \{0,1\}^{256} \rightarrow \mathcal{X}_{l_r}$:
        $$x_r = H_{in}(\mathcal{O}_{r - 1})$$
        \item The leader uses its trapdoor secret key $sk_{l_r}$ to compute the output of VDF and broadcasts it along with the proof to everyone.
        $$(y_r, \pi_r ) = TVDF.TrapdoorEval(pp_{l_r}, x_r, sk_{l_r})$$
        \item As soon as such a message is received, a node checks the correctness
        of the received values using $Verify(pp_{l_r}, x_r, y_r, \pi_r)$. If valid, the beacon output for round $r$ is computed using another hash function $H_{out} : {Y}_{l_r} \rightarrow \mathcal \{0,1\}^{256}$:
        $$\mathcal{O}_r = H_{out}(y_r)$$
        \joenote{I think the hash functions in/out can be skipped for simplicity}
    \end{itemize}
    
    \textbf{Dishonest Leader:} In case the leader does not fulfill its duties as described, the participants make sure that round $r$ is completed with the same output. To achieve this, at the start of each round every non leader node immediately starts to compute the round’s VDF
    output using $(y_r, \pi_r) \leftarrow TVDF.Eval(pp_{l_r}, x_r)$ in the background. Even though computing the VDF without the trapdoor takes at least $T$ sequential steps\joenote{VDF paper uses lowercase t} , it allows recovery in case the leader behaves maliciously. The strong uniqueness property guarantees that the resulting values are always equal to the ones computed by the leader.
...
% problem with VDF-ASICs and deciding T

\section{Commit-Reveal-Punish}
\joenote{I would put this right after commit-reveal}
\label{section:commit-reveal-punish}
A distributed randomness beacon realized using commit-reveal scheme should not allow any collusion among participants to be able to compute the final random output before the reveal phase (unpredictability) and no participant should be able to withhold their committed shares to bias the outcome of the beacon (unbiasability). Realizing this requires us to model the behavior of the participants and the adversary, which can be done as follows:
\joenote{this needs to go way earlier and be general for all protocols}
\begin{itemize}
    \item \textbf{Honest participants and $t$-limited adversary:} The adversary is limited in the maximum number of participants (denoted by $t$) that it can corrupt. The honest participants follow the protocol exactly and only deviate from it if corrupted by the adversary. This model is used in most threshold based cryptosystems (discussed later), where the adversary can corrupt up to $t < n/2$ participants.
    \item \textbf{Rational participants: } \joenote{this is the only part that is specific to this section} We abandon the assumption that participants can be trusted and instead are assumed to be rational profit-maximizing entities, colluding or withholding to increase their profits. This model is used in Commit-Reveal-Punish schemes.
\end{itemize}

Commit-Reveal-Punish schemes assume that all participants are rational. It incentivizes participants to adhere to desired behavior by using an escrow to collect initial deposits from them as part of their commitment. Such an escrow can be realized, for example, as a smart contract in Ethereum \cite{david2020economically} or by using timed commitment in Bitcoin \cite{andrychowicz2014secure}. 
Unpredictability and unbiasability can be achieved in two ways:
\joenote{I would merge this into the next two sections, don't need this much intro text}
\begin{enumerate}
    \item Forcing every participant to reveal their shares by collecting large enough collateral \cite{youcai2017randao, andrychowicz2014secure} such that any malicious behavior would result in the deposits being slashed, making it unprofitable.
    \item The other alternative is to use threshold secret sharing schemes to allow the shared secret to be recovered by a subset of participants. However, realizing this with rational participants (instead of a $t$-limited adversary) requires a mechanism to make colluding before the reveal phase or tampering the liveness by withholding unprofitable. Economically Viable Randomness (EVR) \cite{david2020economically} solves it by introducing a novel ``informing'' mechanism in addition to escrow. 
\end{enumerate}

The two approaches are summarized in the following sections.
...
DKG protocol allows a set of $n$ participants to collectively generate the keys required for the scheme\joenote{for what scheme? Threshold crypto?} (i.e. the group public key, the individual secret keys, and the corresponding public keys) without the help of a trusted party. However, a DKG protocol is more than a secret sharing protocol and has the following properties:
...
\cmidrule(lr){11-12}
 & & & & & & & & & & Optimistic & Worst & & \\
\toprule
Commit-Reveal & \hyperref[subsection:commit-reveal]{1} & Commitment  &  1  & \cmark   & All   &  \xmark  & \xmark    & \cmark   & $O(n)$  & $O(n^3)$   & $O(n^3)$  & Bias & $O(1)$ \\ 
\midrule
