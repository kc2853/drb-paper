...
Generating periodic trustworthy randomness is crucial to applications ranging from sampling ballots for recounts in electronic voting \cite{adida2008helios} and choosing winning numbers in gambling and lottery services \cite{bonneau2015bitcoin} to leader election in Proof of Stake blockchains \cite{gilad2017algorand, kiayias2017ouroboros}, blockchain sharding \cite{al2017chainspace, kokoris2018omniledger}, and selecting parameters for cryptographic protocols \cite{baigneres2015trap, lenstra2015random}. The process of generating tamper-proof randomness is nontrivial, as obtaining access to good sources of randomness, even in terms of entropy alone, can be prone to errors.
...
\textbf{Contribution.} The goal of the paper is to systematize the current progress on the problem of generating reliable distributed randomness and devise a theoretical framework and generalization encompassing all distributed randomness protocols in the landscape. To aid comparison and discussion of properties, we provide an overview of these protocols along with the various cryptographic building blocks used to construct them. Drawing from a body of knowledge based on heuristics, we identify two key components of a DRB's design: selection of entropy providers and beacon output generation, which can be decoupled from each other. Enabling a more holistic analysis of a DRB as a result, we also provide new insights and discussion on potential attack vectors, countermeasures, and techniques that lead to better scalability.

\textbf{Paper organization.} We organize the paper as follows. Preliminaries including our system model, a strawman DRB under perfect synchrony (an ideal assumption), commit-reveal, and the definition of an ideal DRB are described in Section \ref{section:preliminaries}. Section \ref{section:vdf} introduces protocols using verifiable delay functions \cite{boneh2018verifiable}, which help achieve both security and simplicity. Then from Section \ref{section:commit-reveal-punish} to \ref{section:dvrf}, protocols are introduced in decreasing order of number of nodes providing \textit{marginal entropy} (i.e. randomness that is independently generated at a node level and is broadcast during the newest round of a DRB). Section \ref{section:commit-reveal-punish} and \ref{section:commit-reveal-recover} review protocols in which all nodes must provide marginal entropy while erroneous behaviors are counteracted, respectively, by financial punishment \cite{youcai2017randao, david2020economically} and threshold secret sharing \cite{schoenmakers1999simple, cascudo2017scrape} or threshold encryption \cite{desmedt1990Threshold}. Section \ref{section:committee-based} illustrates protocols that include in the beginning of each round an extra step of committee selection, enabling only a committee (non-empty proper subset) of nodes to provide marginal entropy. In Section \ref{section:dvrf}, we have protocols that do not require any marginal entropy. Lastly, various discussion topics (e.g. attack vectors and an overall comparison of DRBs) are dived into in Section \ref{section:discussions}.

\section{Preliminaries}
\label{section:preliminaries}
We delineate the necessary preliminaries in this section, starting with the description of our system model (including threat model) relevant in all protocols portrayed in this paper (unless stated otherwise). We then introduce a strawman DRB assuming a perfectly synchronous network (with zero latency in message delivery) as well as the classical commit-reveal. Identifying problems in both, we define the security of an ideal DRB.
...
Sourcing distributed randomness assuming perfect synchrony is straightforward. Consider the following ``rock-paper-scissors'' protocol where each participant $i$ broadcasts its \textit{entropy contribution} (i.e. independently generated randomness) $e_i \in \mathbb{N}$ to every other participant at the same time as per simultaneity observed (say) in rock-paper-scissors. Naturally, its protocol output $\mathcal{O}$ can be given by
\[
...
Take a simple scenario without perfect synchrony: three participants $\{P_1, P_2, P_3\}$ coordinate to produce $\mathcal{O} = e_1 + e_2 + e_3$ where each $P_i$ sends $e_i$, and suppose $P_3$ already obtains $e_1$ and $e_2$ (due to non-zero message latency) before sending $e_3$ to $P_1$ and $P_2$ as the last step of the protocol. Then $P_3$ is able to set the output $\mathcal{O}$ to be whichever value $\tilde{\mathcal{O}}$, as it can choose $e_3 = \tilde{\mathcal{O}} - e_1 - e_2$. Effectively, the output is centralized by $P_3$.
...
Clearly, we should prevent anyone from tampering with (e.g. predicting, biasing, or delaying) each beacon output of a DRB. Apart from this, the output should be verifiable by any third party. Based on these requirements, the overall security properties of an ideal DRB are given by the following.
...
While Escrow-DKG is usually used to generate keys for secret sharing, EVR adapts it to realize a DRB by using the group secret (i.e. the implied group secret key $sk$ after a DKG) as $\mathcal{O}_r$. EVR proceeds in five phases---Register, Setup, Commit, Inform, and Reveal.
\begin{enumerate}
    \item \textbf{Register.} Every participant registers by depositing 1 coin per secret (i.e. entropy contribution).
...
\textit{Publicly verifiable secret sharing} (PVSS) extends VSS by allowing any external party to verify the correctness of sharing and reconstruction of a secret via NIZK (non-interactive zero-knowledge) proofs posted by the dealer and the reconstructing participants, respectively. A PVSS scheme can be realized by the algorithms $\mathsf{PVSS} = (\mathsf{Setup}, \mathsf{KeyGen}, \mathsf{Enc}, \mathsf{Dec}, \mathsf{ShareGen}, \mathsf{ShareVerify}, \mathsf{Recon})$. To provide public verifiability, PVSS extends VSS by requiring $\mathsf{PVSS.ShareGen}$ to use keys generated by $\mathsf{PVSS.KeyGen}$ to encrypt and decrypt shares via $\mathsf{PVSS.Enc}$ and $\mathsf{PVSS.Dec}$ as subroutines and to generate auxiliary proofs for public verification as another subroutine. Then $\mathsf{PVSS.ShareVerify}$ can be run by anyone (not just the participants as in VSS). See Appendix \ref{appendix:pvss} for details.
\end{definition}
...
Counteracting the last revealer attack, commit-reveal-recover extends commit-reveal by an additional recovery phase for reconstructing (via PVSS) secrets withheld during reveal. The idea is that the commit phase additionally requires every participant to distribute PVSS shares of its corresponding secret (whose commitment is broadcast in the same phase as usual). The tradeoff is additional communication overhead, which could be amplified multiplicatively in the recover phase if $O(n)$ PVSS reconstructions need to take place. Scrape \cite{cascudo2017scrape} adopts this technique.\\

\noindent\textbf{Scrape.} The main building block of Scrape is its own PVSS scheme \cite{cascudo2017scrape}. The initial setup for Scrape requires generating public keys $pk_i$ for each of the $n$ participants using $\mathsf{PVSS.KeyGen}$. The protocol then proceeds as follows.
\begin{enumerate}
...
The first step of a committee-based DRB protocol involves selecting $\mathcal{C}_r$ in a way agreeable by all nodes. We classify committee selection mechanisms into two: public and private.
...
\item DDH-DRB. Each round of DDH-DRB involves DDH-DVRF. As $\mathsf{PartialVerify}$ and $\mathsf{Verify}$ from Dfinity-DVRF (i.e. each round of Dfinity) rely on verifying pairing equations (see Appendix \ref{appendix:dfinity-dvrf}), the speed at which each beacon output is generated can be made faster if we replace each pairing equation with a DLEQ NIZK achieving the same effect (see Appendix \ref{appendix:dleq}). The tradeoff is space, as $\pi$ of DDH-DVRF grows linearly in the size of $A$. See Appendix \ref{appendix:ddh-dvrf} for details.
...
\midrule
Scrape & \multirow{5}{*}{\ref{section:commit-reveal-recover}} & PVSS & $n/2$ & \xmark & All & 1 & \cmark & \cmark & $O(n^2)$ & $O(n^3)$ & $O(n^4)$ & Predict & $O(n^3)$ \\
Albatross & & PVSS & $n/2$ & \xmark & All & 1 & \cmark & \cmark & $O(1)$ & $O(n)$ & $O(n^2)$ & Predict & $O(n^3)$ \\
RandShare & & (P)VSS & $n/3$ & \xmark & All & 1 & \cmark & \cmark & $O(n^3)$ & $O(n^3)$ & $O(n^4)$ & Predict & $O(1)$ \\
SecRand & & PVSS & $n/2$ & \xmark & All & 1 & \cmark & \cmark & $O(n^2)$ & $O(n^3)$ & $O(n^4)$ & Predict & $O(n^3)$ \\
HERB & & Thr. ElGamal & $n/3$ & \xmark & All & 1 & \cmark & \cmark & $O(n)$ & $O(n^2)$ & $O(n^3)$ & Predict & $O(n^4)$ \\
\midrule
...
BRandPiper & & (P)VSS & $n/2$ & \xmark & Committee\tnote{*} & 1 & \cmark & \cmark & $O(n^2)$ & $O(n^2)$ & $O(n^3)$ & Predict & $O(n^4)$ \\
Ouroboros & & PVSS & $n/2$ & \xmark & Committee & 1 & \cmark & \xmark & $O(n^2)$ & $O(n^3)$ & $O(n^3)$\tnote{†} & Bias\tnote{b} & $O(n^2)$\tnote{†} \\
RandHound & & PVSS & $n/3$ & \xmark & Committee & 1 & \xmark & \xmark & $O(c n)$ & $O(c^2 n)$ & $O(c^2 n^2)$ & Bias & $O(n^3)$ \\
SPURT & & PVSS & $n/3$ & \xmark & Committee & 1 & \xmark & \xmark & $O(n)$ & $O(n^2)$ & $O(n^2)$ & Bias\tnote{b} & $O(n^3)$ \\
...
\item[b] In Ouroboros, it is in theory possible that the entire entropy-providing committee is Byzantine. In SPURT, only the round leader can be honest while all others are Byzantine.
\end{tablenotes}
\end{threeparttable}
\end{table*}

\subsection{Comparison of DRBs}
We provide Table \ref{table:comparison} to offer an overall comparison of DRBs. \textit{Fault Tolerance} indicates the minimum number of faulty nodes that can abort a protocol (after the initial setup). In other words, a protocol with fault tolerance $f$ can tolerate at most $f - 1$ faulty nodes. Protocols with \textit{Independent Participation} allows a node to contribute to beacon output without the knowledge of other participating nodes. However, it differs from a permissionless setting in the sense that a node would still have to register before becoming one of the entropy providers to allow verification of its contribution and prevent grinding attacks. \textit{Verification Complexity} refers to the computational cost for a passive node to verify a beacon output. We exclude the cost associated with the initial setup for both verification and communication complexities. Verification complexities for threshold based protocols are computed assuming a cost of $O(n)$ per Lagrange interpolation. PVSS-based protocols use Scrape's optimization \cite{cascudo2017scrape}. \textit{Communication Complexity} is calculated assuming bitwise point-point communication between the participating nodes by default. We consider a publicly accessible bulletin board (PBB) as a reliable and stable information exchange channel in protocols where it is intrinsic to the protocol's construction. For PBB, both the bitwise communication cost for writing as well as reading data from the bulletin board contribute to the total cost. In the absence of PBB, byzantine consensus incurs a cost of $O(n^2)$ per decision by default. \textit{Max Damage} refers to the maximum damage possible when $n-1$ adversarial nodes cooperate to predict or bias the protocol's output. For escrow-based protocols, we assume the adversaries are rational. Any protocol suffers the most when the adversaries are able to bias the output undetected. \textit{Recovery} refers to the cost associated with recovering from an adversarial corruption, once detected. Running $\mathsf{PVSS.KeyGen}$ for PVSS based protocols, $\mathsf{VDF.Setup}$ for RandRunner, and regenerating keys for private lottery based protocols incurs $O(n^3)$ recovery cost without PBB (and $O(n^2)$ with PBB). Protocols that use DKG for setup have a higher recovery cost of $O(n^4)$ for regenerating the keys.
% \iffalse
...
\section{Conclusion}
In this paper, we provide an overview and comparison of various DRB protocols with a range of cost and quality tradeoffs under different setups, assumptions and adversarial models. We further describe a modular framework comprising of two components: selection of entropy providers and beacon output generation, which can be used to analyze a DRB's design. Based on our systematization, we provide the following insights.
\begin{itemize}
    \item PVSS-based protocols are suitable when high quality of unbiased uniform randomness is to be generated with a dynamic but small set of entropy providers. It can be scaled by using various techniques to select a subset of entropy providers.
    \item Protocols requiring DKG (versus PKI) for setup (DVRF, threshold encryption based) scale better in $n$ but with a relatively static (to avoid a new DKG per reconfiguration) set of entropy providers.
    ...
    \item Protocols based on private lottery are suitable for generating quick but biasable pseudorandomness with a large dynamic set of entropy providers, and allow independent participation.
    % escrow based protocols are suitable when used in conjunction with a lottery/jackpot/ associated with generating randomness.
\end{itemize}
...
\begin{itemize}
    \item Though using repeated modular squaring to provide delay in VDF works well on paper, practical applications of VDFs require a good estimate of the lower bound of wall-clock time required for modular squaring. To guarantee the security of VDF based protocols, designs for efficient modular squaring targeting different hardware is an important area of research.
    ...
    \item With the exception of VDF based protocols like Unicorn++, all other DRB protocols use a permissioned setting which requires the nodes to run an initial setup (PKI, DKG) and establish their identities. Extending these protocols to allow ad-hoc participation requires further research.
...
\begin{itemize}
    ...
    \item $\mathsf{Enc}(pk_i, m) \rightarrow c$ and $\mathsf{Dec}(sk_i, c) \rightarrow m'$ are subalgorithms used when the dealer sends an encrypted share to node $i$ and when node $i$ decrypts the encrypted share, respectively. Both $\mathsf{Enc}$ and $\mathsf{Dec}$ may optionally output a proof (e.g. $\pi_{DLEQ}$).
    \item $\mathsf{ShareGen}(s) \rightarrow (\{\mathsf{Enc}(pk_i, s_i)\}, \{s'_i\}, \pi)$ with $s'_i = \mathsf{Dec}(sk_i, \mathsf{Enc}(pk_i, s_i))$ is a two-part process. First, the dealer with secret $s$ generates secret shares $\{s_i\}$, encrypts them to generate $\{\mathsf{Enc}(pk_i, s_i)\}$, and sends each of them to node $i$ along with an optional encryption proof of correctness. Second, node $i$ decrypts the received encryption from the dealer to generate $s'_i$ and broadcasts it for reconstruction along with an optional decryption proof of correctness. At the protocol level, this yields $\{s'_i\}$ as a result. Note that it is possible that $s'_i \neq s_i$. In fact, $s'_i = h^{s_i}$ for some group generator $h$ is standard in the landscape due to certain PVSS implementation details. $\pi$ incorporates optional encryption and decryption proofs of correctness as well as any auxiliary proof necessary.
    \item $\mathsf{ShareVerify}(\{\mathsf{Enc}(pk_i, s_i)\}, \{s'_i\}, \pi) \rightarrow \{0, 1\}$ verifies if the sharing is correct overall. Again, verifying $\pi$ should involve verifying the optional proofs for encryption and decryption as well as any auxiliary proof necessary.
...
% One of the most common PVSS schemes used in practice is one by Schoenmakers \cite{schoenmakers1999simple}. As typical, the setup involves $g, h \in \mathbb{G}_q$ for $\mathbb{G}_q$ of prime order $q$. Additionally, each participant $P_i$ generates a secret key $x_i \in \mathbb{Z}^*_q$ and registers $y_i = h^{x_i}$ as its public key.
...
\begin{itemize}
    \item $\mathsf{DKG}(1^\lambda, t, n) \rightarrow (sk_i, pk_i, pk)$ runs a typical DKG.
     ...
     \item $\mathsf{DecShare}(sk_i, c) \rightarrow D_i$ takes ciphertext $c = (A, B)$, the individual secret key $sk_i$ and the group public key $pk$ as input and generates the decryption share $D_i = sk_i \cdot A$.
...
Providing a compact proof $\pi$, GLOW-DVRF uses a bilinear pairing $e: \mathbb{G}_1 \times \mathbb{G}_2 \rightarrow \mathbb{G}_T$ similar to BLS (Appendix \ref{appendix:bls}) such that the setup includes hash functions $H_1: \{0, 1\}^* \rightarrow \mathbb{G}_1$ and $H_2: \mathbb{G}_1 \rightarrow \{0, 1\}^{y(\lambda)}$. While the following tuple of algorithms generally resembles that of DDH-DVRF, algebraic modifications are made accordingly due to pairings.
\begin{enumerate}
...
\begin{threeparttable}
\caption{Committee-Based DRB Protocols}
...
\cmidrule{1-5}
\multirow{5}{*}{\spheading[0.45\textwidth]{\mbox{Step 1: Committee Selection}}} & \multirow{3}{*}[-2.8cm]{Public} & RR & \begin{tabular}{@{}l@{}}\textbf{BRandPiper}\\\textit{Step 1}: Node $i \equiv r \pmod n$\\\textit{Step 2}: Share-aggregate-reconstruct\end{tabular} & \\
\cmidrule{3-5}
